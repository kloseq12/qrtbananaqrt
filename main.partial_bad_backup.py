...
CASE_PURCHASE_IMAGES = {
    "daily": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_10-34-30.jpg",
    "homeless": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_08-27-38.jpg",
    "standard": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_10-34-30.jpg",
    "special": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_08-57-48.jpg",
}
CASE_REWARD_IMAGES = {
    "daily": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_10-37-25.jpg",
    "homeless": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_08-27-38.jpg",
    "standard": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_10-34-30.jpg",
    "special": r"C:\Users\user\Pictures\Screenshots\photo_2026-04-29_08-57-48.jpg",
}
...
for f in [BALANCES_FILE, DUELS_FILE, GIVEAWAYS_FILE, PRIZES_FILE, DONATES_FILE, PROMO_FILE]:
...
def _business_product_cost(business_meta: dict) -> int:
    return max(1000, int(business_meta.get("base_income", 0)) // 10)


def _short_business_button_label(name: str, price: int) -> str:
    price_text = format_number(int(price))
    suffix = f" - {price_text}$"
    max_name_len = max(8, 40 - len(suffix))
    short_name = name if len(name) <= max_name_len else name[: max_name_len - 1].rstrip() + "…"
...
async def handlers(message: GroupTypes.MessageEvent):
    ...
    if command == "join_giveaway":
        ...
        if not await is_user_subscribed_to_bot_group(user_id):
            ...
            await bot.api.messages.send(
                peer_id=message.object.peer_id,
                random_id=0,
                message=f"@id{user_id} (пользователь), вы не подписаны на сообщество бота: https://vk.com/club{groupid}",
                disable_mentions=1,
            )
            return True
    ...
    if command == "buybiz_menu":
        ...
        kb = Keyboard(inline=True)
        lines = [f"Выберите бизнес для покупки [{page}/{total_pages}]:", ""]
    ...
    if command == "buy_business":
        ...
        await delete_message(groupid, message.object.peer_id, message.object.conversation_message_id)
        await bot.api.messages.send(peer_id=message.object.peer_id, random_id=0, message=f'Вы приобрели "{info["name"]}" (филиал #{branch_no}).')
        return True
    ...
    if command == "biz_show_branches":
        ...
        for branch in branches:
            kb.add(
                Callback(
                    f'Филиал #{branch["branch_no"]}',
                    {"command": "biz_open", "business_id": branch["id"], "owner_id": user_id, "business_key": business_key},
                ),
                color=KeyboardButtonColor.PRIMARY,
            ).row()
        ...
        kb.add(Callback("Назад", {"command": "biz_menu", "owner_id": user_id}), color=KeyboardButtonColor.SECONDARY)
        await bot.api.messages.send(
            peer_id=message.object.peer_id,
            random_id=0,
            keyboard=kb,
            message=f"Бизнес: {title}\nВыберите филиал:",
        )
        return True
    ...
    if command == "biz_menu":
        ...
        for business_key, branches in grouped.items():
            ...
            branch_balance = sum(int(branch.get("branch_balance", 0)) for branch in branches)
            lines.append(f"• {title}: {len(branches)} шт. | продуктов: {products} | финка: {format_number(branch_balance)}$")
            kb.add(
                Callback(
                    f"{title} ({len(branches)})",
    ...
    if command == "biz_open":
        ...
        kb = Keyboard(inline=True)
        kb.add(
            Callback("Улучшить биз", {"command": "biz_upgrade", "business_id": business_id, "owner_id": user_id}),
            color=KeyboardButtonColor.POSITIVE,
        ).row()
        kb.add(
            Callback("Пополнить продукты", {"command": "biz_refill", "business_id": business_id, "owner_id": user_id, "amount": 10}),
            color=KeyboardButtonColor.PRIMARY,
        ).row()
        kb.add(
            Callback(
        ...
        kb.add(
            Callback(
                "Назад",
                {"command": "biz_show_branches", "business_key": str(payload.get("business_key", biz["business_key"])), "owner_id": user_id},
            ),
            color=KeyboardButtonColor.SECONDARY,
        )
        await bot.api.messages.send(
            ...
            keyboard=kb,
            message=(
                f'Филиал #{biz["branch_no"]}: {biz["meta"]["name"]}\n'
                f'Уровень: {level}\n'
                f'Продукты: {products}\n'
                f'Баланс филиала: {format_number(int(biz["branch_balance"]))}$\n'
                f'Доход за сбор: {format_number(income_preview)}$\n'
    ...
    if command == "biz_sell_confirm":
        ...
        kb.add(
            Callback("Выйти", {"command": "biz_show_branches", "business_key": business_key, "owner_id": user_id}),
            color=KeyboardButtonColor.SECONDARY,
        )
        await bot.api.messages.send(
            peer_id=message.object.peer_id,
            random_id=0,
            keyboard=kb,
            message=f"Вы точно хотите продать {title}?",
        )
        return True
    ...
    if command == "biz_branch_sell_confirm":
        ...
        kb.add(
            Callback("Выйти", {"command": "biz_open", "business_id": business_id, "owner_id": user_id, "business_key": biz["business_key"]}),
            color=KeyboardButtonColor.SECONDARY,
        )
        await bot.api.messages.send(
            peer_id=message.object.peer_id,
            random_id=0,
            keyboard=kb,
            message=f"Вы точно хотите продать филиал #{biz['branch_no']} бизнеса {biz['meta']['name']}?",
        )
        return True
    ...
    if command == "biz_refill":
        if not _callback_owner_allowed(payload, user_id):
            ...
            return True
        business_id = int(payload.get("business_id", 0) or 0)
        amount = max(1, int(payload.get("amount", 10) or 10))
        biz = await get_business_by_id(user_id, business_id)
        if not biz:
            await answer_callback_event(message, "Филиал не найден")
            return True
        ...
        save_data(BALANCES_FILE, balances)
        _drop_user_cache(user_id)
        await answer_callback_event(message, f"Продукты пополнены на {add_amount}")
        payload["command"] = "biz_open"
        payload["business_key"] = biz["business_key"]
        message.object.payload = payload
        return await handlers(message)
    ...
    if command == "getban":
        ...
        # ---- Получаем информацию о блокировках (как в команде /getban) ----
        # Глобальные баны
    ...
    if command_identifier in bot_identifiers:
        ...
        if command in ["баланс"]:
            ...
            await message.reply(
                ...
                f"🏢 Заработок бизнесов за день: {format_number(bal.get('business_income_today', 0))}$\n"
...
# =============================================
        ...
        if command in ["business", "бизнес"]:
            ...
            if not biz_list:
                kb = Keyboard(inline=True)
                kb.add(Callback("Купить бизнес", {"command": "buybiz_menu", "owner_id": user_id}), color=KeyboardButtonColor.PRIMARY)
                await message.reply("🏢 У вас пока нет бизнесов.", keyboard=kb)
                return True
            kb = Keyboard(inline=True)
            kb.add(Callback("Открыть меню бизнесов", {"command": "biz_menu", "owner_id": user_id}), color=KeyboardButtonColor.PRIMARY).row()
            kb.add(Callback("Купить бизнес", {"command": "buybiz_menu", "owner_id": user_id}), color=KeyboardButtonColor.SECONDARY)
            total_branch_balance = sum(int(b.get("branch_balance", 0)) for b in biz_list)
            ...
            lines = [
                ...
                f"Общая финка: {format_number(total_branch_balance)}$",
                "",
        ...
        if command in ["inventory", "inv", "инвентарь", "инвент"]:
            ...
            if len(items) > 25:
                lines.append("")
                lines.append(f"И еще предметов: {len(items) - 25}")
            await message.reply("\n".join(lines))
            return True
        ...
        if command in ["+ппрод", "ппрод"]:
            if len(arguments) < 2 or not str(arguments[1]).isdigit():
                await message.reply("Укажите количество продуктов: +ппрод [количество]")
                return True
            amount = max(1, int(arguments[1]))
            businesses = await get_user_businesses(user_id)
            if not businesses:
            ...
            if not refill_targets:
                await message.reply("Все бизнесы уже заполнены продуктами.")
                return True
            bal = get_balance(user_id)
            if bal["bank"] < total_cost:
                await message.reply(
            ...
            _drop_user_cache(user_id)
            await message.reply(
                f"Продукты пополнены в {filled} бизнес(е/ах).\n"
                f"С банковского счета списано: {format_number(total_cost)}$."
            )
            return True

        if command in ["+собратьфин", "собратьфин"]:
            businesses = await get_user_businesses(user_id)
            if not businesses:
                await message.reply("У вас нет бизнесов.")
                return True
            collected, total_income, _errors = await collect_all_business_income(user_id)
            if collected <= 0:
                await message.reply("Не удалось собрать финку. Проверьте, есть ли продукты в бизнесах.")
                return True
            bal = get_balance(user_id)
            ...
            _drop_user_cache(user_id)
            await message.reply(
                f"Финка собрана с {collected} бизнес(а/ов).\n"
                f"Заработано: {format_number(total_income)}$.\n"
                "Средства зачислены в баланс филиалов. Используйте +снятьфин, чтобы вывести их."
            )
            return True

        if command in ["+снятьфин", "снятьфин"]:
            businesses = await get_user_businesses(user_id)
            if not businesses:
                await message.reply("У вас нет бизнесов.")
                return True
            changed, total = await withdraw_all_business_balance(user_id)
            if total <= 0:
                await message.reply("В бизнесах сейчас нет финки для вывода.")
                return True
            bal = get_balance(user_id)
            bal["wallet"] += total
            balances[str(user_id)] = bal
            ...
            _drop_user_cache(user_id)
            await message.reply(
                f"Вы сняли финку из {changed} бизнес(а/ов).\n"
                f"Получено на руки: {format_number(total)}$."
            )
            return True
