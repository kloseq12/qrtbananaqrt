...
UPGRADE_COSTS = {1: 100000, 2: 250000, 3: 500000}
...
async def get_user_businesses(user_id: int) -> List[Dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur = await db.execute(
            "SELECT id, business_key, branch_no, upgrade_level, products, branch_balance, talisman_active "
...
async def collect_income(user_id: int, business_id: int) -> Tuple[bool, str, int]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE businesses SET products = products - ?, branch_balance = branch_balance + ? "
            "WHERE id = ? AND user_id = ?",
            (products_to_spend, final_income, business_id, user_id),
        )
        await db.commit()
