...
from utils.business import add_business

DB_PATH = "database.db"

CASE_DEFS = {
    "daily": {"name": "Ежедневный кейс", "money_cost": 0, "banana_cost": 0, "daily": True},
    "homeless": {"name": "Кейс Бомжа", "money_cost": 100000, "banana_cost": 0, "daily": False},
    "standard": {"name": "Стандартный кейс", "money_cost": 600000, "banana_cost": 0, "daily": False},
...
1. Ежедневный кейс
- Игровая валюта (10.000-50.000$) — 80%
- VIP на 1 день — 10%
- Редкий предмет (+10% к доходу /приз) — 10%

2. Кейс Бомжа
- Игровая валюта (50.000-250.000$) — 70%
- Бизнес «Кофейня "Гранд Эспрессо"» — 15%
- VIP на 7 дней — 10%
- Редкий предмет (+10% к доходу /приз) — 4%
- Эпический предмет (+25% к доходу /приз) — 1%

3. Стандартный кейс
- Игровая валюта (250.000-1.000.000$) — 60%
- Бизнес «Ресторан "Гурмания"» — 20%
- VIP на 30 дней — 12%
...
4. Особый кейс
- Игровая валюта (1.000.000-3.000.000$) — 40%
- Бизнес «WorldWide Holdings» — 25%
- VIP на 90 дней — 13%
- Редкий предмет (+10% к доходу /приз) — 4%
...
def _pick_case_reward(case_type: str) -> Dict:
    ...
    if case_type == "homeless":
        ...
        if roll < 85:
            return {"type": "business", "business_key": "grand_espresso", "text": 'Комиссионный дом "Второй Шанс"'}
        if roll < 95:
    ...
    if case_type == "standard":
        ...
        if roll < 80:
            return {"type": "business", "business_key": "gourmania", "text": 'Ресторан "Лунный Берег"'}
        if roll < 92:
    ...
    if roll < 65:
        return {"type": "business", "business_key": "worldwide_holdings", "text": 'Инвестхолдинг "Союз Капитал"'}
    if roll < 78:
