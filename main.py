...
for f in [BALANCES_FILE, DUELS_FILE, GIVEAWAYS_FILE, PRIZES_FILE, DONATES_FILE, PROMO_FILE, SUBS_FILE, TOP_VISIBILITY_FILE]:
...
def _lot_time_left_text(ends_at_raw: str) -> str:
    try:
        ends_at = datetime.fromisoformat(str(ends_at_raw))
    except Exception:
        return "неизвестно"
...
def _business_income_per_day(branch: dict) -> int:
    current_products = int(branch.get("products", 0))
    available_collects = current_products // 5
    return _business_income_per_collect(branch) * available_collects
...
def _item_banana_value(item: dict) -> int:
    ...
    return max(1, item_value if item_value > 0 else 1)
...
async def finalize_expired_auctions():
    ...
    async with aiosqlite.connect("database.db") as db:
        ...
        cur = await db.execute(
            "SELECT id, seller_id, item_type, item_name, item_value, current_bid, highest_bidder_id "
            "FROM auction_items WHERE ends_at <= ?",
            (now_iso,),
        )
        expired = [dict(row) for row in await cur.fetchall()]
        if not expired:
            return 0
...
async def handlers(message: GroupTypes.MessageEvent):
    ...
    if command == "biz_menu":
        ...
        kb = Keyboard(inline=True)
        total_daily_income = sum(_business_income_per_day(branch) for branch in businesses)
        lines = [
        ...
        for business_key, branches in page_items:
            ...
            daily_income = sum(_business_income_per_day(branch) for branch in branches)
            lines.append(
                f"{icon} {title}\n"
    ...
    if command == "biz_open":
        ...
        await bot.api.messages.send(
            ...
            message=(
                ...
                f'💼 Накоплено на филиале: {format_number(int(biz["branch_balance"]))}$\n'
                f'📈 Доход за 1 день: {format_number(_business_income_per_day(biz))}$\n'
                ...
                f'📉 За один сбор тратится: 5 продуктов'
            ),
        )
        return True
...
async def on_chat_message(message: Message):
    ...
    if command_identifier in bot_identifiers:
        ...
        if command in ['подписка']:
            ...
            if not await is_user_subscribed_to_bot_group(user_id):
                await message.reply(f"❗ Подпишитесь на сообщество: https://vk.com/club{groupid}", disable_mentions=1)
                return True
            ...
            await message.reply(
                f"✅ @id{user_id} ({await get_user_name(user_id, chat_id)}) вы подписались на сообщество и получили бонус: "
        ...
        if command in ["баланс"]:
            ...
            daily_business_income = sum(_business_income_per_day(branch) for branch in target_businesses)
        ...
        if command in ["показтоп", "showtop"]:
            ...
            top_visibility[user_key] = new_state
...
# =============================================
        ...
        if command in ["распылить", "salvageitem", "dustitem"]:
            ...
            _drop_user_cache(user_id)
            await message.reply(
                f"♻️ Предмет «{item['item_name']}» распылён.\n"
                f"🍌 Получено бананов: {bananas_reward}\n"
                f"🍌 Теперь у вас: {bal['bananas']}"
            )
            return True
        ...
        if command in ["аукцион", "аук", "auction"]:
            ...
            async with aiosqlite.connect("database.db") as db:
                db.row_factory = aiosqlite.Row
                cur = await db.execute(
                    "SELECT id, item_name, current_bid, seller_id, highest_bidder_id, ends_at "
                    "FROM auction_items ORDER BY current_bid DESC, id ASC LIMIT 25"
                )
            ...
            for index, lot in enumerate(lots, start=1):
                ...
                if lot.get("highest_bidder_id"):
                    ...
                    leader_text = f"лидер: {leader_name}"
                lines.append(
                    f"#{lot['id']} • Лот {index}\n"
                    f"🎁 {lot['item_name']}\n"
                    f"💰 Текущая ставка: {format_number(int(lot['current_bid']))}$\n"
                    f"👤 Продавец: {seller_name}\n"
                    f"🏁 {leader_text}\n"
                    f"⏳ До завершения: {_lot_time_left_text(str(lot['ends_at']))}"
                )
            await message.reply("\n".join(lines))
            return True
        ...
        if command in ["выставитьаук", "sellauction"]:
            ...
            if not item:
                await message.reply("Предмет не найден в инвентаре.")
                return True
            ends_at = (datetime.now() + timedelta(hours=3)).isoformat()
            async with aiosqlite.connect("database.db") as db:
                await db.execute(
                    "INSERT INTO auction_items (seller_id, item_type, item_name, item_value, start_bid, current_bid, ends_at) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (user_id, item["item_type"], item["item_name"], int(item["item_value"]), start_bid, start_bid, ends_at),
                )
                await db.commit()
            await message.reply(
        ...
        if command in ["купаук", "buyauction"]:
            ...
            async with aiosqlite.connect("database.db") as db:
                db.row_factory = aiosqlite.Row
                cur = await db.execute(
                    "SELECT id, seller_id, item_name, current_bid, highest_bidder_id, ends_at FROM auction_items WHERE id = ?",
                    (lot_id,),
                )
            ...
            if int(lot["seller_id"]) == user_id:
                ...
                return True
            if datetime.fromisoformat(lot["ends_at"]) <= datetime.now():
                await finalize_expired_auctions()
        ...
        if command in ["собратьбиз", "collectbiz", "+собратьбиз"]:
            ...
            if collected <= 0:
                await message.reply("Не удалось собрать доход. Проверьте, есть ли продукты в бизнесах.")
                return True
            bal = get_balance(user_id)
            ...
            await message.reply(
                ...
                f"🏢 Филиалов обработано: {collected}\n"
