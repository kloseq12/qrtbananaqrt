    ...
    async with aiosqlite.connect("database.db") as db:
        ...
        for lot in expired:
            ...
            if highest_bidder_id:
                ...
                await log_economy(
                    user_id=int(lot["seller_id"]),
                    target_id=int(highest_bidder_id),
                    amount=int(lot["current_bid"]),
                    log=f"получил(-а) {int(lot['current_bid'])}$ за продажу предмета «{lot['item_name']}» на аукционе",
                )
                await log_economy(
                    user_id=int(highest_bidder_id),
                    target_id=int(lot["seller_id"]),
                    amount=int(lot["current_bid"]),
                    log=f"купил(-а) на аукционе предмет «{lot['item_name']}» за {int(lot['current_bid'])}$",
                )
            else:
                await db.execute(
                    "INSERT INTO inventory (user_id, item_type, item_name, item_value) VALUES (?, ?, ?, ?)",
...
async def _build_auction_page(chat_id: int, page: int = 1):
    ...
    lines = [
        f"🏛 Аукцион предметов [{page}/{total_pages}]",
        "",
